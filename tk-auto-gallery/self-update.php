...
function kswtkautogallerf54a_bootstrap( $plugin_file ) {
	static $instances = [];
	...
	$instance = new KSWTKAUTOGALLERF54ASelfUpdate( $plugin_file );
...
class KSWTKAUTOGALLERF54ASelfUpdate {
		...
		public function register_hooks(): void {
			if (
				is_admin()
				&& defined( 'KSWTKAUTOGALLERF54A_SELF_UPDATE_DIAGNOSTICS' )
				&& KSWTKAUTOGALLERF54A_SELF_UPDATE_DIAGNOSTICS
			) {
		...
		public function render_update_refresh_notice(): void {
			if (
				...
				|| ! isset( $_GET['kswtkautogallerf54a_update_state_refreshed'] )
				|| '1' !== (string) $_GET['kswtkautogallerf54a_update_state_refreshed']
			) {
				return;
			}
		...
		public function maybe_refresh_wordpress_update_state(): void {
			...
			if ( isset( $_GET['kswtkautogallerf54a_refresh_update_state'] ) ) {
				$requested_plugin = sanitize_text_field(
					wp_unslash( (string) $_GET['kswtkautogallerf54a_refresh_update_state'] )
				);

				if ( $requested_plugin === $this->plugin_basename ) {
					check_admin_referer( 'kswtkautogallerf54a_refresh_update_state' );
					$this->refresh_wordpress_update_state();

					wp_safe_redirect(
						add_query_arg(
							'kswtkautogallerf54a_update_state_refreshed',
							'1',
		...
		public function filter_plugin_row_meta( $links, $plugin_file ) {
			...
			if ( current_user_can( 'update_plugins' ) ) {
				$refresh_url = wp_nonce_url(
					add_query_arg(
						'kswtkautogallerf54a_refresh_update_state',
		...
		public function register_diagnostics_page(): void {
			add_management_page(
				...
				'Self-Update Diagnose (KornSW)',
				'manage_options',
				'kswtkautogallerf54a-self-update-diagnostics',
		...
		public function render_diagnostics_page(): void {
			...
			if ( isset( $_POST['ksw_force_update_check'] ) ) {
				check_admin_referer( 'kswtkautogallerf54a_force_update_check' );
			...
			$runtime_rows = [
				...
				'Bootstrap-Funktion'        => 'kswtkautogallerf54a_bootstrap',
				'Updater-Klasse'            => 'KSWTKAUTOGALLERF54ASelfUpdate',
			...
			<div class="wrap">
				...
				<form method="post" style="margin: 18px 0;">
					<?php wp_nonce_field( 'kswtkautogallerf54a_force_update_check' ); ?>
